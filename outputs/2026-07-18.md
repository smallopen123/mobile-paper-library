...
```mermaid
flowchart LR N0["输入/场景: 5帧RGB图像 + 语言指令 + 无人机状态历史 + 先前目标框"] N1["核心模块: Qwen3.5 VLM骨干 (冻结, LoRA适配) + 元查询接口"] N2["机制: 空间基础CPT → 三阶段课程SFT → CoT训练 → 闭环RL (EG-FPO)"] N3["输出: 目标可见性 + 当前帧边界框 + 8步4-DoF航点增量动作块"] N4["指标: 开环: ADE, First DE, Final DE, IoU, Vis F1; 闭环: SR, TCR, ATF, ADE, dErr"] N0 --> N1 N1 --> N2 N2 --> N3 N3 --> N4
```
